...
class ChatGPT:
    ...
    def __init__(self, proxy: str = None, cookies: dict = None) -> Any:
        ...
        if proxy:
            self.session.proxies = {"all": proxy}  # format http://user:pass@ip:port

        self.ip_info: list = IP_Info.fetch_info(self.session)
        self.timezone_offset: int = int(
            datetime.now(ZoneInfo(self.ip_info[5])).utcoffset().total_seconds() / 60
        )
        self.reacts: list = [
            "location",
    ...
    def _fetch_cookies(self) -> None:
        ...
        self.data["config"] = [
            4880,
            datetime.now(ZoneInfo(self.ip_info[5])).strftime(
                f"%a %b %d %Y %H:%M:%S GMT%z ({datetime.now(ZoneInfo(self.ip_info[5])).tzname()})"
            ),
            4294705152,
            random(),
    ...
    def get_conduit(self, next: bool = False) -> str:
        ...
        conduit_request: requests.models.Response = self.session.post(
            "https://chatgpt.com/backend-anon/f/conversation/prepare", json=post_data
        )
    ...
    def start_conversation(self, message: str) -> None:
        ...
        print(conversation_request.text)
